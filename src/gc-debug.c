        ...
        for (int i = 0; i < JL_GC_N_POOLS; i++) {
            ...
            while (next) {
                ...
                if (gc_page_data(next) != gc_page_data(last)) {
                    ...
                    assert(last == NULL || pg->osize == page_metadata(last)->osize);
                    ...
                    if ((line >> j) & 1) {
                        ...
                        if (pg->nfree) {
                            ...
                            while (gc_page_data(next) == data) {
                                int obj_id = (((char*)next) - page_begin) / osize;
                                freelist_map[obj_id / 8] |= 1 << (obj_id % 7);
                                next = next->next;
                            }
                        }
                        // valid all of the tags on the page
                        int obj_id = 0;
                        while ((char*)v <= lim) {
                            int in_freelist = freelist_map[obj_id / 8] & (1 << (obj_id % 7));
                            if (!in_freelist) {
